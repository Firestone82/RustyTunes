...
pub async fn check_author_in_same_voice_channel(ctx: Context<'_>) -> Result<bool, MusicBotError> {
    let user_channel_id: Option<ChannelId> = get_user_voice_channel(ctx, &ctx.author().id);
    let bot_channel_id: Option<ChannelId> = get_user_voice_channel(ctx, &ctx.framework().bot_id);
