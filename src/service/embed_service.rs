...
pub trait SendEmbed {
    fn send_context(&self, ctx: Context<'_>, reply: bool, delete_after: Option<u64>) -> impl std::future::Future<Output = Result<Message, MusicBotError>> + Send;

    fn send_channel(&self, http: Arc<Http>, channel: &GuildChannel, delete_after: Option<u64>, message: Option<String>) -> impl std::future::Future<Output = Result<Message, MusicBotError>> + Send;
}
